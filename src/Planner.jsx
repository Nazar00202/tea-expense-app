...
const PRIORITY_META = {
  high: { emoji: "🔴", ua: "Високий", en: "High", color: "#c4622d" },
  medium: { emoji: "🟠", ua: "Середній", en: "Medium", color: "#d4a84a" },
  low: { emoji: "🟢", ua: "Низький", en: "Low", color: "#a09082" },
  високий: { emoji: "🔴", ua: "Високий", en: "High", color: "#c4622d" },
  середній: { emoji: "🟠", ua: "Середній", en: "Medium", color: "#d4a84a" },
  низький: { emoji: "🟢", ua: "Низький", en: "Low", color: "#a09082" },
};
...
export default function Planner({ user }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      tasksQuery,
      (snapshot) => {
        const data = snapshot.docs.map((taskDoc) => ({ id: taskDoc.id, ...taskDoc.data() }));
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      habitsQuery,
      (snapshot) => {
        const data = snapshot.docs.map((habitDoc) => ({ id: habitDoc.id, ...habitDoc.data() }));
  ...
  useEffect(() => {
    if (!pomRunning) { clearInterval(intervalRef.current); return undefined; }
      ...
      setPomTime((timeLeft) => {
        if (timeLeft <= 1) {
          ...
          if (pomMode === "work") {
            ...
            if (nextCount % 4 === 0) { setPomMode("long"); return POMODORO.long; }
            setPomMode("short");
  ...
  const resetPomodoro = (mode) => { setPomMode(mode); setPomTime(POMODORO[mode]); setPomRunning(false); };

  const addTask = async () => {
    if (!user || !taskInput.trim()) return;
    await addDoc(collection(db, "tasks"), {
      text: taskInput.trim(), done: false, priority, date: selectedDate,
      userId: user.uid, userName: user.displayName ?? "", userEmail: user.email ?? "", createdAt: Date.now(),
    });
    setTaskInput("");
  };
  ...
  const toggleTask = async (id, done) => {
    ...
    if (!done) await addXP(user, 10);
  };

  const deleteTask = async (id) => { if (!user) return; await deleteDoc(doc(db, "tasks", id)); };

  const addGoal = async () => {
    if (!user || !goalTitle.trim() || !goalTarget) return;
    await addDoc(collection(db, "goals"), {
      title: goalTitle.trim(), target: Number(goalTarget), current: Number(goalCurrent) || 0,
      cat: goalCategory, unit: goalUnit || "₴", deadline: goalDeadline || null,
      userId: user.uid, userName: user.displayName ?? "", userEmail: user.email ?? "", createdAt: Date.now(),
    });
    setGoalTitle(""); setGoalTarget(""); setGoalCurrent(""); setGoalUnit("₴"); setGoalDeadline("");
  };
  ...
  const deleteGoal = async (id) => { if (!user) return; await deleteDoc(doc(db, "goals", id)); };

  const addHabit = async () => {
    if (!user || !habitName.trim()) return;
    await addDoc(collection(db, "habits"), {
      name: habitName.trim(), icon: habitIcon, streak: 0, lastDone: null, doneToday: false, history: [],
      userId: user.uid, userName: user.displayName ?? "", userEmail: user.email ?? "", createdAt: Date.now(),
    });
    setHabitName("");
  };
  ...
  const toggleHabit = async (habit) => {
    ...
    if (!habit.doneToday) {
      ...
      await updateDoc(doc(db, "habits", habit.id), { doneToday: true, lastDone: todayStr, streak: newStreak, history: [...history, todayStr] });
      return;
    }
    await updateDoc(doc(db, "habits", habit.id), { doneToday: false, streak: Math.max(0, habit.streak - 1), history: history.filter((item) => item !== todayStr) });
  };

  const deleteHabit = async (id) => { if (!user) return; await deleteDoc(doc(db, "habits", id)); };
  ...
  if (!user) {
    return (
      <div className="container page-planner">
        <PageTitle tabId="планер">{copy.title}</PageTitle>
        <div className="card"><p>{copy.loginHint}</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="container">
      ...
      {plannerError ? (
        <div className="card"><p className="empty planner-error">{plannerError}</p></div>
      ) : null}

      {/* Calendar at top — full width */}
      <div className="card calendar-wrap planner-calendar" style={{ marginTop: 16 }}>
        ...
        {/* Agenda blocks under calendar */}
        <div className="day-summary" style={{ marginTop: 16 }}>
          <div className="day-summary-header">
            <span style={{ fontWeight: 600, color: "#f5ede2" }}>{formatDate(lang, selectedDate)}</span>
            <span>{agendaItemsCount} {lang === "en" ? "items" : "елементів"}</span>
          </div>
      ...
      <div className="planner-grid">
        <div className="planner-main">
          ...
          <div className="card">
            ...
            <div className="card progress-wrap planner-inner-card">
              ...
              <div className="progress-bar">
                <div className="progress-fill" style={{ width: tasksTotal ? `${(doneTasks / tasksTotal) * 100}%` : "0%" }} />
              </div>
            </div>
          ...
          <div className="card">
            ...
            <input placeholder={copy.goalTitle} value={goalTitle} onChange={(e) => setGoalTitle(e.target.value)} />

            <div className="form" style={{ marginTop: 10 }}>
              <input type="number" placeholder={copy.goalTarget} value={goalTarget} onChange={(e) => setGoalTarget(e.target.value)} />
              <input type="number" placeholder={copy.goalCurrent} value={goalCurrent} onChange={(e) => setGoalCurrent(e.target.value)} />
              <input placeholder={copy.goalUnit} value={goalUnit} onChange={(e) => setGoalUnit(e.target.value)} />
            </div>

            <div className="form" style={{ marginTop: 10 }}>
              <select value={goalCategory} onChange={(e) => setGoalCategory(e.target.value)}>
                {GOAL_CATEGORIES.map((category) => (
                  <option key={category.value} value={category.value}>{getGoalCategoryLabel(category.value)}</option>
                ))}
              </select>
                  ...
                  return (
                    <div key={goal.id} className={`goal-card ${done ? "goal-done" : ""}`}>
                      <div className="goal-header">
                        ...
                        {goal.deadline && <span className="goal-days">{formatDate(lang, goal.deadline)}</span>}
                      ...
                      <div className="goal-progress">
                        <div className="progress-bar">
                          <div
                            ...
                            style={{ width: `${percent}%`, background: done ? "#d4a84a" : percent > 60 ? "#d4a84a" : "#c4622d" }}
                          />
                        </div>
                      ...
                      {goalUpdate.id === goal.id ? (
                        <div className="form" style={{ marginTop: 10 }}>
                          ...
                          <button type="button" className="cancel-btn" onClick={() => setGoalUpdate({ id: "", value: "" })}>✕</button>
                        </div>
                      ) : (
                        <button type="button" className="update-btn" onClick={() => setGoalUpdate({ id: goal.id, value: String(goal.current || 0) })}>
                          📈 {copy.updateGoal}
                        </button>
                      )}
                    </div>
        ...
        <div className="planner-side">
          <div className="card pom-card">
            ...
            <div className="pom-modes">
              <button type="button" className={pomMode === "work" ? "pom-mode-btn pom-mode-active" : "pom-mode-btn"} onClick={() => resetPomodoro("work")}>{copy.work}</button>
              <button type="button" className={pomMode === "short" ? "pom-mode-btn pom-mode-active" : "pom-mode-btn"} onClick={() => resetPomodoro("short")}>{copy.shortBreak}</button>
              <button type="button" className={pomMode === "long" ? "pom-mode-btn pom-mode-active" : "pom-mode-btn"} onClick={() => resetPomodoro("long")}>{copy.longBreak}</button>
            </div>

            <div className="pom-time">{pomMinutes}:{pomSeconds}</div>

            <div className="pom-controls">
              <button type="button" className="pom-btn" onClick={() => setPomRunning((v) => !v)}>
                {pomRunning ? copy.pause : copy.start}
              </button>
              <button type="button" className="pom-btn pom-reset" onClick={() => resetPomodoro(pomMode)}>{copy.reset}</button>
            </div>
          </div>
        </div>
      </div>
